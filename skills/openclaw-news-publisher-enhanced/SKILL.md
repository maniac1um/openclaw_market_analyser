...
# OpenClaw News Publisher Skill（增强包）

主流程依赖 `<SKILL_ROOT>/scripts/`、`tools/`、`config/`。鉴权、报告 schema、安全门见 [`../_shared/`](../_shared/)。用户级白名单路线图见 [`../_shared/workspace-api-roadmap.md`](../_shared/workspace-api-roadmap.md)。**OpenClaw 运行时缓存**见 §13。

## 必须遵守的安全准则

通用基线见 [`../_shared/agent-safety-baseline.md`](../_shared/agent-safety-baseline.md)；公网见 [`../_shared/public-deploy-security.md`](../_shared/public-deploy-security.md)。

**本技能附加**：

- 发布报告前 **必须** 委派 `openclaw-report-security`（路径 B，见 public-deploy-security 发布路径表）。
- **禁止编造** 新闻 `url`；爬虫仅 HTTP、不执行 JS。
- **禁止** 多用户共写同一 `<SKILL_ROOT>/config/whitelist.json`（见 §0）。

---

## 0. 多用户鉴权（必读）

鉴权与隔离见 [`../_shared/multi-user-auth.md`](../_shared/multi-user-auth.md)；资源归属见 [`../_shared/ownership-policy.md`](../_shared/ownership-policy.md)。

**本技能特例**：`whitelist.json` 在技能包目录内，多用户共用同一 OpenClaw 实例时勿写入用户级密钥；`monitor_id` 须与当前 per-user Key 配对。对话入口：`openclaw-conversational-assistant`。
...
- 用户要做“价格监测/趋势”（不是新闻报道），并需读已入库价格（见 §8.6 交叉引用 → `openclaw-price-ingest-external`）。
...
2. 编辑产出 JSON（§5.1 必做项）。
3. 委派 `openclaw-report-security` 校验 → §8 POST → 轮询 → §12 回执。

**路径 B — 需要先积累白名单 URL**

1. §7 `tools/cli.py init` 或 `discover` / `refresh`（读 `seed_urls.json`，写 `whitelist.json`）。
2. `list` 或 `suggest` 得到若干 `https://...` 根地址。
3. 将 URL 作为 §6 的 seed，跑爬虫 → §5.1 → `openclaw-report-security` → §8。

**路径 C — 已有符合 §5 的 JSON**

1. 委派 `openclaw-report-security` 校验 → §8 POST → 轮询 → §12。

### 4.2 逐步清单（与路径无关的后半段）

1. **收集**：`BASE_URL`（如 `http://127.0.0.1:8000`，无尾斜杠）、**per-user** `X-Api-Key`（门户账户页生成）、用户 `keyword`、业务 `task_id`、时间范围（写入 `time_range`）。
...
5. **安全门**：委派 Skill **`openclaw-report-security`** 完成 Schema / Ownership / Security / URL / Publish 五项校验；未通过则**不得** POST。
6. **POST**：`POST {BASE_URL}/api/v1/openclaw/reports`，记录 `ingest_id`。
7. **轮询**：`GET {BASE_URL}/api/v1/openclaw/reports/{ingest_id}`，间隔 1–3s，直到 `published` 或 `failed`（建议总超时 60–300s，视部署而定）。
8. **回执**：§12；失败附 HTTP 状态与 body 摘要（密钥脱敏）。
9. **清理磁盘（推荐必做，尤其小盘环境）**：在步骤 8 已完成、且**无需保留本次中间 JSON** 时，立即执行 §13 的 `skill_cleanup.py`（可选用 `--prune-whitelist-history` 收缩 `whitelist.json` 内历史日志）。
...
入站 JSON 须符合 [`../_shared/report-schema.md`](../_shared/report-schema.md)（对齐服务端 `OpenClawReportIn`）。安全约束见 [`../_shared/report-security.md`](../_shared/report-security.md)。

**发布前**须委派 **`openclaw-report-security`** 通过五项校验；完整示例见 `report-schema.md`。
...
**前置条件**：Skill **`openclaw-report-security`** 五项校验已通过（见 [`../_shared/report-security.md`](../_shared/report-security.md)）。

- **POST** `{BASE_URL}/api/v1/openclaw/reports`
- **成功**：HTTP **202**，JSON 含 `ingest_id`、`status`（常见初始为 `queued`）。
- **请求头**：
  - `Content-Type: application/json`
  - `X-Api-Key: <per-user API Key，门户账户页生成>`
...
本技能主路径为 **新闻抓取 + 报告发布**。监测 API 完整说明已拆至专项 Skill：

| 需求 | 委派 Skill |
|------|------------|
| bootstrap、外采 ingest、external-heartbeat | [`openclaw-price-ingest-external`](../openclaw-price-ingest-external/SKILL.md) |
| 多窗口 summary / timeseries 读库 | [`openclaw-price-analysis-reporting`](../openclaw-price-analysis-reporting/SKILL.md) §4.1 |
| 端点速查 | [`openclaw-conversational-assistant/references/api-quickref.md`](../openclaw-conversational-assistant/references/api-quickref.md) |

**生成报告前常用读接口**（多用户模式下须 `X-Api-Key` 或 Bearer）：

```bash
curl -sS "$BASE_URL/api/v1/public/monitoring/monitors" -H "X-Api-Key: ${API_KEY}"
curl -sS "$BASE_URL/api/v1/public/monitoring/<monitor_id>/timeseries?window_days=30" -H "X-Api-Key: ${API_KEY}"
curl -sS "$BASE_URL/api/v1/public/monitoring/<monitor_id>/observations?limit=200" -H "X-Api-Key: ${API_KEY}"
```

默认 **`OPENCLAW_MONITORING_ALLOW_SERVER_SCRAPE=false`**：价格在 OpenClaw 侧采集后 **`POST .../observations/ingest`** 入库。Legacy 服务端 `run-once` / 内建 scheduler 见仓库根目录 `README.md`。
