...
通用基线见 [`../_shared/agent-safety-baseline.md`](../_shared/agent-safety-baseline.md)；HTTP 429 等见 [`../_shared/public-deploy-security.md`](../_shared/public-deploy-security.md)。

**本技能附加**：

- **禁止编造价格**；未确认汇率不得长期自动猜汇率。
- **尊重 robots 与站点条款**；遇 429 停止对该源轰炸（见 public-deploy-security 429 模板）。
- **`raw_payload` 禁止** 含 API Key；`source_url` 须 http(s)。
- bootstrap / 高频 ingest 前见 [`../_shared/quota-policy.md`](../_shared/quota-policy.md)。
...
**本技能特例**：`observations/ingest` 的 `monitor_id` 须属于当前 Key 用户，否则 **404**；`raw_payload` 禁止含 API Key。bootstrap / 高频 ingest 前见 [`../_shared/quota-policy.md`](../_shared/quota-policy.md)。
...
| **`API_KEY`** | 门户 **账户 → API Key 管理** 生成的 **per-user** 请求头 **`X-Api-Key`**（Legacy 全局 Key 仅过渡期 ADMIN） |
...
| HTTP 401 / 403 | API Key 错误或非本用户 monitor | 在门户重新生成 per-user Key；核对 `monitor_id` 归属 |
