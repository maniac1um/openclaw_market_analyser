...
- 不伪造新闻、价格与时间戳；数据不足时 **低置信度**，禁止强单边结论。
- 路径 B 发布：`openclaw-report-security` 强制门；路径 D（`news-trigger` `publish:true`）须先校验 `monitor_id` 归属（见 public-deploy-security 路径 D）。
- **`analysis`、`generated_title` 禁止 HTML**（仅 Markdown 纯文本）。

---

## 0. 多用户鉴权（必读）

鉴权见 [`../_shared/multi-user-auth.md`](../_shared/multi-user-auth.md)；归属见 [`../_shared/ownership-policy.md`](../_shared/ownership-policy.md)。

**本技能特例**：路径 B 发布报告前须委派 **`openclaw-report-security`**；`news-trigger` 且 `publish:true` 时服务端组装报告，仍须确保输入 `monitor_id` 属当前用户。
...
见 [`../_shared/multi-user-auth.md`](../_shared/multi-user-auth.md)。报告入站还须 **`X-Request-Id`**（缺失 **400**）及可选 **`X-Signature`**（见 [`docs/02-backend/api.md`](../../docs/02-backend/api.md)）。发布前 **`openclaw-report-security`** 强制门。
...
- `publish: true` 时会在服务端组装 **`OpenClawReportIn`** 并入队（路径 **D**，见 [`../_shared/public-deploy-security.md`](../_shared/public-deploy-security.md)）。
- **发布前**：Agent 须 `GET /public/monitoring/monitors` 确认 `monitor_id` 属当前用户；禁止对他人 monitor 触发 `publish:true`。
...
| 接口 | Key | 说明 |
|------|-----|------|
| `GET /api/v1/public/monitoring/monitors` | **是** | 列表：`monitor_id`、`keyword`、`observation_count`、`last_captured_at` 等 |
| `GET /api/v1/public/monitoring/{monitor_id}/timeseries?window_days={1-365}` | **是** | 按日聚合点（默认 `window_days=30`） |
| `GET /api/v1/public/monitoring/{monitor_id}/observations?limit={1-1000}` | **是** | 明细行；**当前实现为按 `captured_at` 升序取前 `limit` 条**（时间轴上偏早的一段，未必是「最近」）。做 **近期拐点** 时请优先 **`summary` + `timeseries`**，或明确知晓该排序语义后再用 observations。 |
...
- **`POST /api/v1/openclaw/reports`** + `X-Api-Key` + **`X-Request-Id`** + JSON body（见 [`../_shared/report-schema.md`](../_shared/report-schema.md)）。
- **发布前**须委派 **`openclaw-report-security`** 五项校验通过。
...
字段定义与必填规则见 [`../_shared/report-schema.md`](../_shared/report-schema.md)；发布前安全见 [`../_shared/report-security.md`](../_shared/report-security.md)。

**本技能特有 — `items` 填充建议**：从新闻库选取 3～12 条关键新闻，每条映射为 `NewsItem`（`source` ← `source_name`，`url` ← `source_url`）；`published_at` 优先用新闻 `published_at`，否则 `created_at`。`sources` 建议含 `monitoring-summary`、`news-library`、`public-reports-history`。`analysis` 须含 §5.3～5.6 内容。推荐在扩展字段或 `task_id` 中携带 `monitor_id` 便于审计。
...
开始
  ...
  -> 委派 openclaw-report-security 校验
  -> 可选 POST 发布与状态轮询
结束
```
...
- [ ] 若发布：委派 openclaw-report-security 校验 → 生成 X-Request-Id → POST /openclaw/reports → 轮询至 published 或 failed
...
| 现象 | 处理方向 |
|------|----------|
| `401` / 鉴权失败 | 检查 per-user `X-Api-Key`；勿使用已撤销 Key |
...
见 [`../_shared/agent-safety-baseline.md`](../_shared/agent-safety-baseline.md)；发布前配额见 [`../_shared/quota-policy.md`](../_shared/quota-policy.md)。
...
- 入站 API：[`docs/02-backend/api.md`](../../docs/02-backend/api.md)
