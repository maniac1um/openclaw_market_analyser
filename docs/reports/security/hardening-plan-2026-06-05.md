...
> **报告快照（2026-06-05）** — 加固计划已闭环；pytest 等数字为历史值，勿作当前运维唯一依据。活跃运维见 [gateway-isolation.md](../../human/security/gateway-isolation.md)。
...
- **公网 / 生产**：✅ Phase 1~4 已验证；见 `docs/human/deployment/production.md` checklist
...
| 文档 | `docs/human/api/openclaw-intake.md`、`docs/human/deployment/production.md` 已同步门户 Cookie 鉴权 |
...
| **G-01** | **Gateway 权限越权（USER=admin device）** | ✅ **已修复** | 双 Agent + 双 device + `GatewayPermissionChecker` + 审计；见 [gateway-isolation.md](gateway-isolation.md) |
...
- [x] `docs/human/api/openclaw-intake.md` 鉴权表已同步
...
| `docs/human/deployment/production.md` | 生产部署 + `OPENCLAW_PRODUCTION` checklist |
| `docs/human/api/openclaw-intake.md` | 门户 workflow 鉴权表（已同步） |
