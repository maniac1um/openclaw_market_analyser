...
> **多用户 SaaS（2026-06）**：除 `/healthz` 外，读写 API 须 **JWT**（门户）或 **per-user `X-Api-Key`**（Agent）。Key 在门户 **账户 → API Key 管理** 生成。Legacy 全局 Key 默认关闭（`OPENCLAW_LEGACY_API_KEY_ENABLED=false`）。详见 [migration-plan-2026-06-05.md](../../archive/multi-user/migration-plan-2026-06-05.md)（归档）与 Skill `skills/_shared/multi-user-auth.md`。
...
除健康检查外，**不再**假设 public GET 无鉴权可读全库。详见 [migration-plan-2026-06-05.md](../../archive/multi-user/migration-plan-2026-06-05.md) 与 Skill [`../../../skills/_shared/multi-user-auth.md`](../../../skills/_shared/multi-user-auth.md)。
