...
class MonitoringService:
    ...
    def ensure_tables(self) -> None:
        ...
        with self._connect() as conn, conn.cursor() as cur:
            ...
            cur.execute("ALTER TABLE price_monitors ADD COLUMN IF NOT EXISTS user_id UUID")
            cur.execute("ALTER TABLE price_monitor_urls ADD COLUMN IF NOT EXISTS user_id UUID")
            cur.execute("ALTER TABLE price_observations ADD COLUMN IF NOT EXISTS user_id UUID")
            conn.commit()
    ...
    def bootstrap_monitor(
        ...
        VALUES (%s::uuid, %s, %s, 'openclaw_auto', %s::timestamptz, %s::uuid)
        """
        sql_url = """
        INSERT INTO price_monitor_urls (monitor_id, platform, url, discovered_at, metadata_json, user_id)
        VALUES (%s::uuid, %s, %s, %s::timestamptz, %s::jsonb, %s::uuid)
        ON CONFLICT (monitor_id, url) DO NOTHING
        """
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(sql_monitor, (monitor_id, keyword, cadence, discovered_at, user_id))
    ...
    def ingest_openclaw_observation(
        ...
        *,
        ...
        sql_check = """
        SELECT user_id FROM price_monitors WHERE monitor_id = %s::uuid LIMIT 1
        """
        sql_sel_url = """
        ...
        sql_ins_url = """
        INSERT INTO price_monitor_urls (monitor_id, platform, url, discovered_at, metadata_json, user_id)
        VALUES (%s::uuid, %s, %s, NOW(), %s::jsonb, %s::uuid)
        ON CONFLICT (monitor_id, url) DO NOTHING
        """
        sql_ins_obs = """
        INSERT INTO price_observations (
            monitor_id, monitor_url_id, captured_at, title, price, currency, status, error, raw_payload, user_id
        ) VALUES (%s::uuid, %s, %s, %s, %s, %s, 'ok', NULL, %s::jsonb, %s::uuid)
        RETURNING id
        """
        ...
        with self._connect() as conn, conn.cursor() as cur:
            ...
            if not mon_row:
                raise KeyError(monitor_id)
            mon_user_id = str(mon_row[0]) if mon_row[0] else None
            if user_id and mon_user_id and mon_user_id != user_id:
                raise KeyError(monitor_id)
    ...
    def add_urls(self, monitor_id: str, urls: list[str], platform: str) -> int:
        ...
        for item in urls:
            if not item or not str(item).strip():
                continue
            try:
                cleaned.append(validate_outbound_http_url(str(item).strip()))
            except ValueError:
                continue
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(sql_check, (monitor_id,))
