...
class MonitoringObservationIngestRequest(BaseModel):
    ...
    def validate_raw_payload_size(self) -> "MonitoringObservationIngestRequest":
        if self.raw_payload is not None:
            encoded = json.dumps(self.raw_payload, ensure_ascii=False)
            if len(encoded) > 32_768:
                raise ValueError("raw_payload serialized size must be <= 32768 bytes")
        return self
...
class MonitoringAddUrlsRequest(BaseModel):
    ...
    def validate_urls(cls, values: list[str]) -> list[str]:
        ...
        return [validate_outbound_http_url(item) for item in values if item and item.strip()]
